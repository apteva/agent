...
func NewDiscoveryService(cfg *config.AgentsConfig, agentID, agentName, agentDescription, agentURL string, features map[string]bool, mcpServers []string) (DiscoveryService, error) {
	...
	switch method {
	case "file":
		return NewFileDiscovery(cfg, agentID, agentName, agentDescription, agentURL, features, mcpServers)
	case "mdns":
		if cfg.Group == "" {
